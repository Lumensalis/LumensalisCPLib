...
class SliderControl {
  constructor(domNode) {
    ...
    console.log('creating SliderControl')
  ...
  moveSliderTo(value) {
    ...
    let valueOutput = value.toFixed(2) //+ this.labelCelsiusAbbrev;

    let valueText = value.toFixed(2) //+ this.labelCelsius;

    this.valueNode.textContent = valueOutput;
    this.sliderNode.setAttribute('aria-valuenow', value.toFixed(2));
...
window.addEventListener('load', function () {
  var sliders = document.querySelectorAll('.slider-control');
  for (let i = 0; i < sliders.length; i++) {
    new SliderControl(sliders[i]);
  }
});
